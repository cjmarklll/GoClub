---
title: "GoClub"
---

# GoClub

GoClub 是一个面向技术面试准备与系统复习的内容站点，重点整理高频面试真题、常见八股总结、优质资料视频、项目推荐，以及视频配套文章。

这个项目使用 Hugo 生成静态页面，通过 GitHub Pages 对外发布，当前主题为 `hugo-book`。整体目标不是做传统博客，而是沉淀一套更适合面试复习、查漏补缺和系统整理的知识内容库。

## 适合怎么用

- 如果你正在准备面试，可以先从 `面试真题` 和 `八股总结` 两个栏目开始。
- 如果你想找学习索引、资料和网站，可以直接看 `资源荟萃`。
- 如果你想一起完善内容，直接进入上面的 GitHub 仓库入口或 [投稿与提交流程]({{< relref "/docs/blog/提交流程.md" >}})。

## 站点特色

- 按栏目整理，适合查漏补缺和阶段性复习
- 基于 Markdown 和 GitHub 协作，方便持续补充内容
- 内容结构更偏知识库，而不是零散博客文章
- PR 被合并后，站点会自动部署，贡献者也会同步展示出来

## 站点栏目

GoClub 目前主要覆盖以下内容：

- [面试真题](./docs/interview/)
- [八股总结](./docs/baguwen/)
- [资源荟萃](./docs/resources/)
- [配套文章](./docs/companion/)
- [技术博客](./docs/blog/)
- [贡献者](./docs/contributors/)

## 当前技术方案

- 静态站点生成器：Hugo
- 部署平台：GitHub Pages
- 页面主题：hugo-book
- 内容格式：Markdown

如果你是第一次进入这个站点，可以先从任意栏目开始浏览，后续内容会逐步补全。
